...
from hydra_base.db.model import Template, TemplateType, TypeAttr, Network, Dataset
...
def get_template_as_xml(template_id, **kwargs):
    ...
    template_i = db.DBSession.query(Template).filter(
        Template.id == template_id).options(
            joinedload(Template.templatetypes).joinedload(TemplateType.typeattrs)\
            .joinedload(Template.default_dataset).joinedload(Dataset.metadata)
        ).one()
        ...
        if type_name in existing_types:
            ...
            type_i = db.DBSession.query(TemplateType).filter(
                TemplateType.id == type_id).options(
                    joinedload(TemplateType.typeattrs).joinedload(TypeAttr.attr)).one()
